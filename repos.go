...
func GetCommitRepoList(ctx context.Context, client *github.Client, repoList []*github.Repository) ([]*github.Repository, error) {
	var forksList []*github.Repository
	commitRepoList := repoList
	...
	for _, fork := range forksList {
		for i := 0; i < len(commitRepoList); i++ {
			if fork.GetFullName() == commitRepoList[i].GetFullName() {
				lastElement := (len(commitRepoList) - 1)
				commitRepoList[i] = commitRepoList[lastElement]
				commitRepoList = commitRepoList[:lastElement]
			}
		}
	}
	return commitRepoList, nil
}